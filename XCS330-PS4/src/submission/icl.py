...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    ### START CODE HERE ###
    permutations = np.random.permutation(len(support_inputs))
    for idx in permutations:
        support_input = support_inputs[idx]
        support_label = support_labels[idx]

        if prompt_mode == 'none':
            prompt += support_input + ' ' + support_label + ' '
        elif prompt_mode == 'tldr':
            prompt += support_input + ' TL;DR: ' + support_label + '. '
        elif prompt_mode == 'qa':
            prompt += support_input + ' In the ' + support_label + '. '
        elif prompt_mode == 'custom':
            prompt += 'Article: ' + support_input + ' Summary: ' + support_label + '. '
        else:
            print(f'prompt_mode [{prompt_mode}] is not supported.')
            break
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    with torch.inference_mode():
        for _ in range(max_tokens):
            ...
            logits = outputs.logits                                 # 1 * prefix_len * vocab_size
            logits_of_next_token = logits[0][-1]                    # 1 * vocab_size
            next_token_id = torch.argmax(logits_of_next_token)

            # add next_token_id to input_ids
            ...
            # add next_token_id to sampled_tokens
                ...
                for k in ks:
                    ...
                    for repeat in range(repeats):
                        ...
                        for row in pbar:
                            ...
                            input_ids = tokenizer(icl_prompt).input_ids
                            input_ids = torch.as_tensor(input_ids).reshape(1, -1).to(DEVICE)
                            sample_tokens = do_sample(model, input_ids, stop_tokens, max_tokens)
                            decoded_prediction = tokenizer.decode(sample_tokens)
                            ### END CODE HERE ###
