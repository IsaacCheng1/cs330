...
class MultiTaskNet(nn.Module):
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        # initialize each layer except the final layer
        for layer_idx in range(len(layer_sizes) - 1):
            curr_layer_size = layer_sizes[layer_idx]
            next_layer_size = layer_sizes[layer_idx + 1]
            mlp_layers.append(nn.Linear(in_features=curr_layer_size, out_features=next_layer_size))
            mlp_layers.append(nn.ReLU())

        # initialize the final layer
        mlp_layers.append(nn.Linear(in_features=layer_sizes[-1], out_features=1))
        ### END CODE HERE ###
        return mlp_layers
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        ### START CODE HERE ###
        user_reps = self.U(user_ids)
        item_reps = self.Q(item_ids)
        user_multi_item_reps = user_reps * item_reps

        user_bias = self.A(user_ids).reshape(-1)
        item_bias = self.B(item_ids).reshape(-1)

        predictions = torch.sum(user_multi_item_reps, dim = 1).reshape(-1) + user_bias + item_bias

        x = torch.cat((user_reps, item_reps, user_multi_item_reps), dim = 1)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        user_fact_reps = self.U_fact(user_ids)
        item_fact_reps = self.Q_fact(item_ids)
        user_multi_item_fact_reps = user_fact_reps * item_fact_reps

        user_bias = self.A(user_ids).reshape(-1)
        item_bias = self.B(item_ids).reshape(-1)

        predictions = torch.sum(user_multi_item_fact_reps, dim=1).reshape(-1) + user_bias + item_bias

        user_reg_reps = self.U_reg(user_ids)
        item_reg_reps = self.Q_reg(item_ids)
        user_multi_item_reg_reps = user_reg_reps * item_reg_reps
        x = torch.cat((user_reg_reps, item_reg_reps, user_multi_item_reg_reps), dim = 1)
        for layer in self.mlp_layers:
            x = layer(x)
        score = x.reshape(-1)
        ### END CODE HERE ###
