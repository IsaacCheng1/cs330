...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            # compute features using support examples
            features_support = self._network(images_support)   # (K * N, D)

            # compute prototypes
            N = torch.max(labels_support).item() + 1
            K = features_support.shape[0] // N
            D = features_support.shape[-1]
            prototypes = torch.zeros(N, D)                     # (N, D)
            for i in range(N):
                prototypes[i] = torch.mean(features_support[i * K: (i+1) * K, :], dim=0)

            # calculate distance and accuracy for query examples
            features_query = self._network(images_query)                      # (Q * N, D)
            dists_query = torch.cdist(features_query, prototypes, p=2)        # (Q * N, N)
            dists_query = -torch.square(dists_query)

            loss = F.cross_entropy(dists_query, labels_query)   # compute loss
            loss_batch.append(loss)

            logits_query = nn.Softmax(dim=1)(dists_query)                     # (Q * N, N)
            accuracy_query = util.score(logits_query, labels_query)

            accuracy_query_batch.append(accuracy_query)

            # calculate distance and accuracy for support examples
            dists_support = torch.cdist(features_support, prototypes, p=2)    # (K * N, N)
            dists_support = -torch.square(dists_support)

            logits_support = nn.Softmax(dim=1)(dists_support)
