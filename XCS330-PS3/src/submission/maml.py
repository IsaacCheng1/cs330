...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps):
            ...
            grads = autograd.grad(loss, list(parameters.values()), create_graph=train)

            # update the parameters
            i = 0
            adapted_parameters = {}
            for k, v in parameters.items():
                adapted_parameters[k] = v - self._inner_lrs[k] * grads[i]
                i += 1
            parameters = adapted_parameters

            # compute accuracy using the adapted network parameters
            logits = self._forward(images, parameters)
            accuracies.append(util.score(logits, labels))
        ### END CODE HERE ###
